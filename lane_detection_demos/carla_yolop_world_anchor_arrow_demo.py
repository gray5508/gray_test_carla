"""
carla_yolop_world_anchor_arrow_demo.py

Realtime CARLA + YOLOP lane-segmentation navigation hint demo with a
world-anchored ground AR arrow.

Compared with carla_yolop_ground_arrow_demo.py:
  - Ground demo: each frame draws the arrow in the current vehicle frame.
  - This demo: when an arrow is first locked, its ground polygon is converted
    to CARLA world coordinates. Later frames project that fixed world marker
    using the current camera pose, so it behaves like a road sign painted on
    the road while the ego vehicle moves.
...
def choose_straight_target(points, start, width, height, args):
    ...
    # vehicle-forward line from about 10m to 15m, and only allow it when YOLOP's
...
def main():
    ...
    # Mirrors lesson 08's camera setup. In world-anchor mode, the first stable
    # arrow is converted from ego-local ground coordinates into CARLA world
    # coordinates, then reprojected from the current camera pose every frame.
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        ...
                        if set_nav_from_key(event, tracker):
            ...
            if camera.latest_rgb is not None:
                ...
                if args.arrow_projection == "world":
                    ...
                    if world_anchor is not None:
                        ...
                        drew_world = draw_world_anchored_arrow(bgr, world_anchor, camera_transform, now, args)
